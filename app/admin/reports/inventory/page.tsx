                ...
                <div className="max-w-6xl mx-auto">
                    ...
                    <div id="report-content" className="bg-white rounded-[40px] p-8 md:p-12 shadow-xl border border-gray-100 print:shadow-none print:border-none print:p-0">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-gray-100 pb-8 mb-8">
                            <div>
                                <div className="flex items-center gap-2 text-[#2d5a41] mb-2">
                                    ...
                                    <span className="text-xl font-black uppercase tracking-widest bubbly-text">Prime Addis</span>
                                </div>
                                <h1 className="text-4xl font-black text-slate-900">Inventory <span className="text-[#2d5a41]">Net Worth</span></h1>
                                <p className="text-gray-500 font-medium mt-1 uppercase tracking-wider text-sm">Financial Valuation Report</p>
                            </div>
                            <div className="text-right mt-4 md:mt-0">
                                <p className="text-sm font-bold text-gray-400 uppercase">Date Generated</p>
                                <p className="text-lg font-bold text-[#2d5a41]">{today}</p>
                                <p className="text-xs text-gray-400 font-medium mt-1 italic">System generated admin report</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                            <div className="bg-[#2d5a41] rounded-3xl p-6 text-white shadow-lg shadow-[#2d5a41]/20">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="text-xs font-bold uppercase tracking-widest opacity-70">Total Valuation</span>
                                </div>
                                <div className="text-4xl font-black mb-1">{totalNetWorth.toLocaleString()} Br</div>
                                <div className="text-sm opacity-80 font-medium">Estimated Market Value</div>
                            </div>
                            ...
                            <div className="bg-[#f5bc6b] rounded-3xl p-6 text-[#1a1a1a] shadow-lg shadow-[#f5bc6b]/20">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="text-xs font-bold uppercase tracking-widest opacity-60">Total Items</span>
                                </div>
                                <div className="text-4xl font-black mb-1">{stockItems.length}</div>
                                <div className="text-sm opacity-70 font-medium">Unique Stock SKUs</div>
                            </div>
                            ...
                            <div className="bg-blue-500 rounded-3xl p-6 text-white shadow-lg shadow-blue-500/20">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="text-xs font-bold uppercase tracking-widest opacity-70">Top Category</span>
                                </div>
                                <div className="text-4xl font-black mb-1 truncate">{categorySummary[0]?.name || "N/A"}</div>
                                <div className="text-sm opacity-80 font-medium">Highest Asset Value</div>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {categorySummary.map((cat, idx) => (
                                    <div key={idx} className="bg-gray-50 rounded-2xl p-4 border border-gray-100 group hover:border-[#2d5a41]/30 transition-colors">
                                        ...
                                        <p className="text-xl font-black text-slate-800">{cat.value.toLocaleString()} Br</p>
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-2 font-bold uppercase">{cat.count} Items ({((cat.value / totalNetWorth) * 100).toFixed(1)}%)</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {stockItems.map((item, idx) => (
                                            <tr key={idx} className="group hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="py-4 px-2 text-right font-medium text-gray-500">{(item.unitCost ?? 0).toLocaleString()} Br</td>
                                                <td className="py-4 px-2 text-right font-black text-slate-800">{((item.quantity ?? 0) * (item.unitCost ?? 0)).toLocaleString()} Br</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                    <tfoot>
                                        <tr className="border-t-2 border-slate-900">
                                            <td colSpan={4} className="py-6 text-right font-black text-slate-400 uppercase tracking-widest text-xs">Total Estimated Asset Value</td>
                                            <td className="py-6 text-right font-black text-2xl text-[#2d5a41]">{totalNetWorth.toLocaleString()} Br</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                        ...
                        <div className="mt-16 pt-8 border-t border-gray-100 text-center text-gray-400 text-xs font-semibold uppercase tracking-[0.2em] print:mt-8">
                            Confidential Business Report &copy; {new Date().getFullYear()} Prime Addis Coffee
                        </div>
                    </div>
                </div>
